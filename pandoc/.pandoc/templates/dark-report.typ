...
#let horizontalrule = [
  ...
  #block(spacing: 1em, inset: (5pt))[
    // #v(1pt)
    #line(start: ( 2 * 12pt,0%), end: (100%,0%), stroke: 1pt + rgb("#bfbdb6"))
    // #v(1pt) 
  ]
...
#set block(
	...
	// above: 0.96*12pt, below: 0.96*12pt,
	//for testing
	// inset: (top: 0.96*12pt, bottom: 0.96*12pt),
...
#show heading: it => {
  it;parbreak();v(0.5* 12pt, weak: true)
}
#show heading.where( level: 1): it => {
  set text(size: 20pt)
  set block(above: 18pt, below: 4pt + 4pt) // default: 1.2 em
  it
  }
#show heading.where( level: 2): it => {
  set text(size: 16pt)
  set block(above: 8pt + 9pt, below: 4pt + 4pt)
  it
}
#show heading.where( level: 3): it => {
  set text(size: 14em/12)
  set block(above: 8pt + 9pt, below: 4pt + 4pt)
  it
}
#show heading.where( level: 4): it => {
  set text(size: 1em)
  set block(above: 2pt + 9pt, below: 1pt + 4pt)
  it
}
...
#show heading.where( level: 6): it => {
  set text(size: 1em)
  set block(above: 1pt + 9pt, below: 0pt + 4pt)
  it
}
...
#show quote: it => {
  ...
  show box: set pad(left: 2 * 12pt, right: 0pt)
  set text(
    size: 1em,
    fill:  rgb("#bfbdb6"),
    style: "italic", // 五号
  ...
  let block-content=box.with(
    ...
    outset: (left: -2 * 12pt, rest: 0pt), // negative out set aglin quote's left padding
...
#show image: it => {
  ...
  // set block(
  //   above: 0pt, below: 0pt, inset: (top: 10pt, bottom: 0pt),
...
#show figure: it => {
  ...
  set block(above: 10pt, below: spacing_below,
    // inset: (top: 10pt, bottom: spacing_below),
...
#let callout(title: "Callout", title-color: white, body-color: white, icon: none, body) = {
    ...
    let title_font_size = 1em
    let content_font_size = 1em // same as blockquote
...
// text tracking -0.02cm
  if title != none {
    align(center)[
      #block(
        inset: (top: 1.44 * 12pt, bottom: 1.44 * 12pt),
        // height: 1em,
        below: 4pt,
        above: 0pt,
      )[
...
#let customer_bottom_right_display(
  customer: none,
  version: none,
) = {
  let actual_version = if version == none {"Draft"} else { version }
  if customer != none {
    place(bottom + right)[
      #block(
        inset: (right: 1.44 * 12pt, bottom: 1.44 * 12pt),
        // height: 1em,
        below: 4pt,
        above: 0pt,
      )[
...
#let colorize_cvss(in_text) = {
  if in_text != none {
  let parts = in_text.text.split(" ")
  let score = float(parts.at(1))
...
#let ajust_pagesize = (paper: "a4", pagesize: (:), it) => {
  ...
  if "width" not in pagesize.keys() and "height" not in pagesize.keys() {
    if paper != none {
      ...
      set page(width: 595.28pt, height: 841.89pt, )
      it
      print_pagesize()
    } 
    return
...
// Document Element settings
  ...
  set page(
    ...
    footer-descent: 0em,
    // names of authors and date in page1 header if no title
    header: context {
      let loc = here()
      // in header 
  ...
  if title != none { 
    ...
    customer_bottom_right_display(customer: customer_name, version: version)
  }
  pagebreak()
...
$if(company_contacts)$
  company_contacts: (
$for(company_contacts)$
$if(company_contacts.name)$
    ( name: [$company_contacts.name$],
      affiliation: [$company_contacts.affiliation$],
